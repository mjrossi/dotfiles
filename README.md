...
Zellij doesn't support config includes, so we use a shared config file and machine-specific overrides.

**Setup on a new machine:**

The `install.py` script automatically creates `~/.config/zellij/config.kdl` from `config.shared.kdl` if it doesn't exist. If you need machine-specific settings (like cert paths), edit the file:
...
- `config.kdl` is automatically created from `config.shared.kdl` during installation (not tracked in git)
- Any existing `config.kdl` is backed up before installation
- Edit `config.kdl` for machine-specific settings without affecting the shared config
...
- `~/.config/zellij/config.kdl` - Machine-specific Zellij configuration

Use the corresponding `.template` files as references when setting up a new machine.
